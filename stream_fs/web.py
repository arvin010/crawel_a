...
class RequestHandler(object):
    ...
    def generate_token(self, isremember:bool,username:str=None,passwd :str=None,flag=True):
        payload = { 'id': '{}'.format(uuid.uuid4()),
                    'exp': datetime.datetime.utcnow() + eval(self.application.settings.get('jwt_expire',None)),
                    'iat': datetime.datetime.utcnow()}
        username=base64.b64decode(self.user.lower()).decode('utf8') if not username else username
        payload['key'] = self.hcrypto(key=f'{username}@&{passwd}', msg=self.application.settings.get('cookie_secret',None), flag=flag)
        b64_token = jwt.encode(payload, self.application.settings.get('secret_key',None), algorithm="HS256")
        expire = 365 * 24 * 60 * 60 if isremember else 30 * 24 * 60 * 60
        self.redis.setex('{}'.format(username), expire, b64_token)
        return b64_token
    ...
    def hcrypto(self, key, msg, flag=True):
        ts_byte = msg.encode("utf-8")
        crypto = hmac.new(key.encode("utf-8"), ts_byte, digestmod=sha3_512).hexdigest() if flag else hmac.new(
            key.encode("utf-8"), ts_byte, digestmod=sha1).hexdigest()
        return crypto
    ...
    @property
    def redis(self, db=15):
        return Redis(db).conn()
    ...
    def authentic(self) -> str:
        try:
            client = bonsai.LDAPClient("ldap://ldap.cmc-xinnuo.com:389")
            client.set_credentials("SIMPLE", user=f"CMC\{base64.b64decode(self.user).decode('utf8')}", password=f"{base64.b64decode(self.passwd).decode('utf8')}")
            with client.connect() as conn:
                self.who=conn.whoami()
